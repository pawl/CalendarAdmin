...
class Calendar(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    calendar_id = db.Column(db.String(255))  # Identifier of the calendar.
    summary = db.Column(db.String(255))  # Title of the calendar. Read-only.
    timezone = db.Column(db.String(255))  # The time zone of the calendar. Optional. Read-only.
    ...
    users = db.relationship('User', secondary=users_and_calendars,
                            backref=db.backref('calendars', lazy='dynamic'))  # many-to-many relationship
...
class Event(db.Model):
    __tablename__ = 'event'
    id = db.Column(db.Integer, primary_key=True)
    summary = db.Column(db.String(255), nullable=False)  # Title of the event.
